...
### Trigger
API Gateway -- POST/PUT/DELETE /locations/{locationId}/menu/{proxy+} -- Auth: JWT

### Purpose
Staff-facing menu-item writes. The {proxy+} catch-all dispatches create,
update, and delete operations from the method and remaining path. Protected
GET requests are handled by the read-only get-menu Lambda.
...
Create requires key nonexistence; update/delete load strongly consistently and
condition on the complete state loaded so concurrent changes return `409`.
Ambiguous DynamoDB write outcomes are reconciled with a strongly consistent
read and at most one idempotent retry before returning success, `409`, or a
sanitized `503`.
...
- [ ] Internal dispatch on `(method, proxy_path)` covers every write action this route family needs to support
